...
function Luna:Notification(data) -- action e.g open messages
    ...
    local asset = GetIcon(data.Icon, data.ImageSource)
    if type(asset) == "table" then
      newNotification.Icon.Image = 'rbxassetid://'..asset.id
      newNotification.Icon.ImageRectOffset = asset.imageRectOffset
			newNotification.Icon.ImageRectSize = asset.imageRectSize
    else
      newNotification.Icon.Image = asset
    end
...
function Luna:CreateWindow(WindowSettings)
    ...
    local asset = GetIcon(TabSettings.Icon, TabSettings.ImageSource)
    if type(asset) == "table" then
      TabButton.ImageLabel.Image = 'rbxassetid://'..asset.id
      TabButton.ImageLabel.ImageRectOffset = asset.imageRectOffset
      TabButton.ImageLabel.ImageRectSize = asset.imageRectSize
    else
      TabButton.ImageLabel.Image = asset
    end
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateLabel(LabelSettings)
				...
				if LabelSettings.Icon ~= nil and LabelSettings.ImageSource ~= nil then
					local asset = GetIcon(LabelSettings.Icon, LabelSettings.ImageSource)
					if type(asset) == "table" then
